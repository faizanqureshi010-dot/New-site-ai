...
export function WorkflowExplorer({ stages, autoAdvance = true }: { stages: WorkflowStage[]; autoAdvance?: boolean }) {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
        ...
        {stages.map((stage, i) => {
          ...
          return (
            <button
              ...
              onClick={() => setActive(i)}
              className={cn(
                'flex flex-shrink-0 items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm transition-all lg:flex-shrink',
                isActive ? 'border-blue/50 bg-panel2 text-ink' : 'border-transparent text-slate hover:border-line hover:bg-panel2/50'
              )}
            >
      ...
      <div className="relative rounded-2xl border border-line bg-panel2 p-8">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {categories[active].items.map((item) => (
            <div key={item} className="rounded-xl border border-line bg-panel2 px-5 py-4 text-sm text-ink">
              {item}
            </div>
          ))}
        </motion.div>
