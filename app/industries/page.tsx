...
export const metadata: Metadata = {
  title: 'Solutions',
  description:
    'One 8-agent AI front office platform, deployed as Dental Automated, Medical Automated, AISC Booking Agent, or scoped to your industry — automotive, hospitality, dining, and other appointment-driven businesses.',
};
...
const deployments = [
  ...
  },
  {
    ...
    icon: Stethoscope,
    blurb: 'The full 8-agent team, configured for a dental front office — connected directly into the PMS you already use.',
  },
  {
    name: 'Medical Automated',
    href: '/products/medical-automated',
    icon: HeartPulse,
    blurb: 'The same 8-agent team, configured for a medical front office — connected directly into the EHR you already use.',
  },
];
    ...
    <>
      ...
      <main id="main">
        <HeroLayout
          ...
          description="The same eight-agent team, configured differently depending on what you run. Dental and Medical Automated deploy the full team. AISC Booking Agent is one agent, standalone. Every other industry gets exactly the agents that fit — not one generic assistant stretched thin."
          ...
          visual={
            <div className="grid w-full max-w-sm grid-cols-2 gap-3">
              {[...deployments.slice(0, 1), ...industries].map((item) => {
        ...
        <ContentSection background="panel">
          <RevealContainer className="mb-10">
            <SectionHeader
              ...
              title="Dental and medical practices run all eight agents."
        ...
        <ContentSection>
          <RevealContainer className="mb-10">
            <SectionHeader
              ...
              description="Each carries different urgency, different vocabulary, and a different definition of a successful outcome. These industries deploy the specific agents that fit their volume — not the full eight by default."
            />
          </RevealContainer>
          <StaggerGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {industries.map((ind, i) => {
              const Icon = ind.icon;
              return (
                <StaggerItem key={ind.name} className={i % 2 === 1 ? 'sm:mt-8' : ''}>
